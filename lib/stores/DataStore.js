'use strict';

/**
 * @fileOverview
 * Based store for all DataStore classes.
 *
 * @author Ben Stahl <bhstahl@gmail.com>
 */

const Uid = require('../models/Uid');
const File = require('../models/File');
const EventEmitter = require('emittery');
const ERRORS = require('../constants').ERRORS;
const EVENTS = require('../constants').EVENTS;
const debug = require('debug');
const log = debug('tus-node-server:stores');
const _ = require('lodash');

const UPLOAD_PROGRESS_THROTTLE_INTERVAL = 1000;
const UPLOAD_SPEED_THROTTLE_INTERVAL = 1000;

class DataStore extends EventEmitter {
    constructor(options) {
        super();
        if (!options || !options.path) {
            throw new Error('Store must have a path');
        }
        if (options.namingFunction && typeof options.namingFunction !== 'function') {
            throw new Error('namingFunction must be a function');
        }
        this.path = options.path;
        this.generateFileName = options.namingFunction || Uid.rand;
        this.relativeLocation = options.relativeLocation || false;
    }

    get extensions() {
        if (!this._extensions) {
            return null;
        }
        return this._extensions.join();
    }

    set extensions(extensions_array) {
        if (!Array.isArray(extensions_array)) {
            throw new Error('DataStore extensions must be an array');
        }
        this._extensions = extensions_array;
    }

    /**
     * Called in POST requests. This method just creates a
     * file, implementing the creation extension.
     *
     * http://tus.io/protocols/resumable-upload.html#creation
     *
     * @param  {object} req http.incomingMessage
     * @return {Promise}
     */
    create(req) {
        return new Promise(async(resolve, reject) => {
            const upload_length = req.headers['upload-length'];
            const upload_defer_length = req.headers['upload-defer-length'];
            const upload_metadata = req.headers['upload-metadata'];

            if (upload_length === undefined && upload_defer_length === undefined) {
                return reject(ERRORS.INVALID_LENGTH);
            }

            const file_id = this.generateFileName(req);
            const file = new File(file_id, upload_length, upload_defer_length, upload_metadata);

            await this.emit(EVENTS.EVENT_FILE_CREATED, { file });
            return resolve(file);
        });
    }

    /**
     * Called in PATCH requests. This method should write data
     * to the DataStore file, and possibly implement the
     * concatenation extension.
     *
     * http://tus.io/protocols/resumable-upload.html#concatenation
     *
     * @param  {object} req http.incomingMessage
     * @return {Promise}
     */
    write(req) {
        log('[DataStore] write');
        return new Promise(async(resolve, reject) => {
            // Stub resolve for tests
            const offset = 0;

            await this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, { file: null });
            return resolve(offset);
        });
    }

    /**
     * Called in HEAD requests. This method should return the bytes
     * writen to the DataStore, for the client to know where to resume
     * the upload.
     *
     * @param  {string} id     filename
     * @return {Promise}       bytes written
     */
    getOffset(id) {
        return new Promise((resolve, reject) => {
            if (!id) {
                return reject(ERRORS.FILE_NOT_FOUND);
            }

            return resolve({ size: 0, upload_length: 1 });
        });
    }

    /**
     * Returns a file metadata by file_id
     *
     * @param  {string} file_id     filename
     * @return {Promise}       file metadata
     */
    getMetadata(file_id) {
        return new Promise((resolve, reject) => {
            if (!file_id) {
                return reject(ERRORS.FILE_NOT_FOUND);
            }

            return resolve({ size: 0, upload_length: 1 });
        });
    }

    /**
     * Called in DELETE requests. This method just removes a
     * file, implementing the termination extension.
     *
     * https://tus.io/protocols/resumable-upload.html#termination
     *
     * @param  {object} req http.incomingMessage
     * @param  {string} file_id name of the file
     * @return {Promise}
     */
    remove(req, file_id) {
        log('[DataStore] delete');
        return new Promise(async(resolve, reject) => {
            // Stub resolve for tests
            await this.emit(EVENTS.EVENT_FILE_REMOVED, { file: null });
            return resolve();
        });
    }

    _getUploadingProgressEmitter() {
        const storeContext = this;
        return _.throttle(async(fileConfig, offset, needToAwait) => {
            if (needToAwait) {
                await storeContext.emit(EVENTS.EVENT_UPLOAD_PROGRESS, { file: Object.assign(fileConfig, { offset }) });
            }
            else {
                storeContext.emit(EVENTS.EVENT_UPLOAD_PROGRESS, { file: Object.assign(fileConfig, { offset }) });
            }
        }, UPLOAD_PROGRESS_THROTTLE_INTERVAL);
    }

    _getRemainingUploadTimeEmitter() {
        const storeContext = this;
        return _.throttle(async(fileConfig, remaining_time) => {
            storeContext.emit(EVENTS.EVENT_UPLOAD_REMAINING_TIME, { file: Object.assign(fileConfig, { remaining_time }) });
        }, UPLOAD_SPEED_THROTTLE_INTERVAL);
    }
}

module.exports = DataStore;
