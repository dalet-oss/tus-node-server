...
class FileStore extends DataStore {
    ...
    create(req) {
        return new Promise((resolve, reject) => {
            ...
            return fs.open(`${this.directory}/${file.id}`, 'w', (err, fd) => {
                ...
                return fs.close(fd, async(exception) => {
                    if (exception) {
                    ...
                    try {
                        ...
                        log('[FileStore] create: Error', e);
                        fs.unlinkSync(`${this.directory}/${file.id}`);
                        return reject(e);
                    }
                    return resolve(file);
                });
    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            req.on('data', (buffer) => {
                ...
                if (config) {
                    progressEmitter(config, new_offset);
                }
            });

            req.on('close', async() => {
                if (config) {
                    progressEmitter(config, new_offset, true);
                    await progressEmitter.flush();
                    this.emit(EVENTS.EVENT_UPLOAD_SUSPENDED, { file: config });
                }
            });
            ...
            stream.on('error', async(e) => {
                ...
                if (config) {
                    progressEmitter(config, new_offset, true);
                    await progressEmitter.flush();
                    this.emit(EVENTS.EVENT_FILE_ERROR, { error: e });
                }
            ...
            return req.pipe(stream).on('finish', async() => {
                ...
                if (config && parseInt(config.upload_length, 10) === offset) {
                    progressEmitter(config, new_offset, true);
                    await progressEmitter.flush();
                    await this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, { file: config });
                }
                resolve(offset);
            });
        });
    ...
    remove(req, file_id) {
        return new Promise((resolve, reject) => {
            const path = `${this.directory}/${file_id}`;
            const config = this.configstore.get(file_id);
            return fs.unlink(path, async(error) => {
                if (error && error.code === FILE_DOESNT_EXIST) {
                    log(`[FileStore] remove: No file found at ${path}`);
                    return reject(ERRORS.FILE_NOT_FOUND);
                }
                ...
                if (config) {
                    await this.emit(EVENTS.EVENT_FILE_REMOVED, { file: config });
                }
                return resolve();
            });
