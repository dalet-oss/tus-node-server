    ...
    _uploadPart(metadata, read_stream, current_part_number, initial_offset) {
        ...
        return new Promise((resolve, reject) => {
            ...
            read_stream.on('data', (chunk) => {
                ...
                if (Date.now() - startUploadingPartTimestamp > 1000) {
                    const totalUploadedLength = uploadedLength + chunkAccumulator.length;
                    const remainingTime = this._getRemainingTimeInSec(startUploadingPartTimestamp, totalUploadedLength, metadata.file.upload_length, chunkAccumulator.length);
