...
const MB = 1024 * 1024;
const OPTIMAL_PART_SIZE = 10 * MB;
const APPROXIMATION_INDEX = 1000;

let s3UploadingSpeed;
...
class S3Store extends DataStore {
     ...
     * @param  {Object}          parsedMetadata     parsed file metatada
    ...
    // TODO need to decompose the method
    /**
    ...
    _uploadPart(metadata, read_stream, current_part_number, initial_offset) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const remainingUploadTimeEmitter = this._getRemainingUploadTimeEmitter();
            ...
            let startUploadingPartTimestamp = Date.now();
            let remainingTime;
            read_stream.on('data', (chunk) => {
                ...
                if (chunkAccumulator.length > MB || (Date.now() - startUploadingPartTimestamp > 1000)) {
                    const totalUploadedLength = uploadedLength + chunkAccumulator.length;
                    remainingTime = this._getRemainingTimeInSec(
                        startUploadingPartTimestamp,
                        totalUploadedLength,
                        metadata.file.upload_length,
                        chunkAccumulator.length
                    );
                    remainingUploadTimeEmitter(metadata.file, remainingTime);
                }
                ...
                if (chunkAccumulator.length > chunkSize) {
                    ...
                    const s3UploadStartTimestamp = Date.now();
                    const remainingTimeEmitterInterval = this._emittingRemainingTimeDuringUploadingPartToS3(
                        remainingUploadTimeEmitter,
                        metadata.file,
                        chunkAccumulator.length,
                        remainingTime
                    );
                    this.client.uploadPart({
                        ...
                        .then((data) => {
                            ...
                            this._updateS3UploadingSpeed(s3UploadStartTimestamp, chunkAccumulator.length);
                            clearInterval(remainingTimeEmitterInterval);
                            startUploadingPartTimestamp = Date.now();
                            read_stream.resume();
            ...
            read_stream.on('close', async() => {
                ...
                if (metadata.file && parseInt(metadata.file.upload_length, 10) > uploadedLength) {
                    remainingUploadTimeEmitter(metadata.file, undefined);
                    progressEmitter(metadata.file, uploadedLength, true);
                    await progressEmitter.flush();
            ...
            read_stream.on('end', () => {
                if (chunkAccumulator) {
                    ...
                    return this.client.uploadPart({
                        ...
                        .then(async(data) => {
                            // resume to read the next chunk
                            this._updateS3UploadingSpeed(uploadStartTimestamp, chunkAccumulator.length);
                            clearInterval(remainingTimeEmitterInterval);
                            read_stream.resume();
                            log(`[${metadata.file.id}] finished uploading part #${current_part_number}`);
                            current_part_number++;
    ...
    /**
     * Set interval for emitting remaining time event during uploading part to S3:
     *
     * @param {Object}             remainingUploadTimeEmitter       remaining upload time event emitter
     * @param {Object}             file                             file
     * @param {Number}             s3PartSize                       length of the uploading to S3 part
     * @param {Number}             prevRemainingTime                previous value of the remaining time
     * @return {Interval}                                           returns interval that you have to clear after S3 uploading will be finished
     */
    _emittingRemainingTimeDuringUploadingPartToS3(remainingUploadTimeEmitter, file, s3PartSize, prevRemainingTime) {
        let estimatedPartUploadingTime = Math.ceil(s3PartSize / s3UploadingSpeed);
        return setInterval(() => {
            if (estimatedPartUploadingTime >= 0 && prevRemainingTime > 0) {
                prevRemainingTime--;
                estimatedPartUploadingTime--;
            }
            if (prevRemainingTime > 0) {
                remainingUploadTimeEmitter(file, prevRemainingTime);
            }
        }, 1000);
    }

    /**
     * Updates S3 uploading speed:
     *
     * @param {Number}             startTimestamp                start S3 upload part timestamp
     * @param {Number}                partSize                      length of the uploaded size
     */
    _updateS3UploadingSpeed(startTimestamp, partSize) {
        const lastSpeed = partSize * 1000 / (Date.now() - startTimestamp);
        if (!s3UploadingSpeed) {
            s3UploadingSpeed = lastSpeed;
        }
        const approximatedSpeed = ((s3UploadingSpeed * APPROXIMATION_INDEX) + lastSpeed) / (APPROXIMATION_INDEX + 1);
        s3UploadingSpeed = Math.round(approximatedSpeed);
    }

    /**
     * Returning estimated remaining time in seconds:
     *
     * @param {Number}             startUploadToBackendTimestamp         Start upload part timestamp
     * @param {Number}                totalUploadedLength                   length of uploaded file part
     * @param {Number}                totalLength                           length of the file
     * @param {Number}                uploadedPartLength                    length of the uploaded part
     * @return {Number}                                                     minimum required part size
     */
    _getRemainingTimeInSec(
        startUploadToBackendTimestamp,
        totalUploadedLength,
        totalLength,
        uploadedPartLength
    ) {
        const remainingUploadLength = totalLength - totalUploadedLength;
        const remainingS3UploadingTime = s3UploadingSpeed ? remainingUploadLength / s3UploadingSpeed : 0;
        const uploadingSpeed = (uploadedPartLength * 1000 / (Date.now() - startUploadToBackendTimestamp));
        const remainingUploadingTime = remainingUploadLength / uploadingSpeed;
        return Math.ceil(remainingS3UploadingTime + remainingUploadingTime);
    }
