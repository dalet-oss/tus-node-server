...
class TusServer extends EventEmitter {
    ...
    constructor() {
        ...
        this.on(EventEmitter.listenerRemoved, (event) => {
            this.datastore.removeListener(event.eventName, event.listener);
            REQUEST_METHODS.forEach((method) => {
                this.handlers[method].removeListener(event.eventName, event.listener);
            });
        });
        ...
        this.on(EventEmitter.listenerAdded, (event) => {
            this.datastore.on(event.eventName, event.listener);
            REQUEST_METHODS.forEach((method) => {
                this.handlers[method].on(event.eventName, event.listener);
            });
        });
    }
